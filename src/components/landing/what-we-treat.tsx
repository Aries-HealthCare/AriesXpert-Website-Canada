...
const treatmentAreas = [
  {
    ...
    items: [
      ...
      { name: "ACL & Meniscus Ligament Tears" },
      ...
      { name: "Whiplash & MVA Trauma (OCF-18 / ICBC)" },
      { name: "Concussion Rehabilitation (CCMI)" },
      { name: "Tennis & Golfer's Elbow" },
      { name: "Ankle Sprains & Chronic Instability" },
  ...
  },
  {
    ...
    basePath: "/conditions",
    items: [
      { name: "Radiating Nerve Pain & Numbness" },
      { name: "Morning Joint Stiffness" },
      { name: "Muscle Spasms & Tightness" },
      { name: "Joint Clicking & Crepitus" },
      ...
      { name: "Limited Shoulder Range of Motion" },
      { name: "Post-Surgical Muscular Weakness" },
      { name: "Post-Concussion Dizziness & Headaches" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Dry Needling & Intramuscular (IMS)" },
      { name: "3D Kinematic Motion Analysis" },
      { name: "Joint Mobilization & Manipulation" },
      { name: "Progressive Resistance Loading" },
      { name: "Bobath Neurological Therapy" },
      ...
      { name: "Pelvic Floor & Core Stabilization" },
    ].map(item => ({ ...item, slug: toSlug(item.name) }))
  }
];
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10 space-y-16">
        <div className="max-w-4xl mx-auto text-center space-y-6 flex flex-col items-center animate-reveal-up">
          ...
          <p className="text-base md:text-lg text-muted-foreground leading-relaxed max-w-3xl mx-auto font-light">
            Evidence-informed physical therapy pathways combining hands-on clinical therapy, 3D anatomical biomechanics, and active motor retraining across Canada.
          </p>
        </div>
