import { CanadianProvinces, IndianAreaType, IndianCityType, IndianStateType } from './locations';
...
function buildLocationIndex(): LocationPageData[] {
    ...
    for (const province of CanadianProvinces) {
        if (!province.seoEnabled) continue;
        ...
        for (const city of province.cities) {
            ...
            for (const area of city.areas) {
                ...
                pages.push({
                    ...
                    metaTitle: `Best In-Home Physiotherapy in ${area.name}, ${city.name} | Aries PhysioCare Canada`,
                    metaDescription: `Expert registered physiotherapy at home in ${area.name}, ${city.name}. CPO/CPTBC certified clinicians, direct billing to Sun Life, Manulife & Canada Life. Same-day visits.`,
                    keywords: [
                    ...
                    heroSubheading: `Hospital-grade clinical physical therapy delivered to your doorstep in ${area.name}, ${city.name}. Registered with Canadian provincial colleges.`,
                ...
                if (area.subAreas) {
                    for (const sub of area.subAreas) {
                        ...
                        pages.push({
                            ...
                            metaTitle: `In-Home Physiotherapy in ${sub.name}, ${area.name} | Aries PhysioCare Canada`,
                            metaDescription: `Certified home physiotherapy in ${sub.name}, ${area.name}, ${city.name}. Direct billing and same-day clinical visits.`,
                            keywords: [`physiotherapy in ${sub.name.toLowerCase()}`, `physio ${sub.name.toLowerCase()}`],
                            heroHeading: `Home Physiotherapy in ${sub.name}`,
                            heroSubheading: `Specialized rehabilitation in ${sub.name}, ${area.name}. CPO/CPTBC registered practitioners.`,
